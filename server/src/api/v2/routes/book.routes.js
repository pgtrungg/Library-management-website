...
router.route('/')
    ...
    .post(csrf.csrfMiddleware,auth.adminMiddleware, upload.single('cover'), imageFilterMiddleware, bookController.create); // POST /api/v2/books - Create a new Book (Admin)

router.route('/:bookId')
    .get(bookController.findOne) // GET /api/v2/books/:bookId - Retrieve a single Book with id (Public)
    .put(csrf.csrfMiddleware,auth.adminMiddleware, bookController.update) // PUT /api/v2/books/:bookId - Update a Book with id (Admin)
    .delete(csrf.csrfMiddleware,auth.adminMiddleware, bookController.delete); // DELETE /api/v2/books/:bookId - Delete a Book with id (Admin)
...
router.route('/:bookId/reviews')
    ...
    .post(csrf.csrfMiddleware,auth.authMiddleware, bookController.addReview); // POST /api/v2/books/:bookId/reviews - Add a Review to a Book with id (User)
